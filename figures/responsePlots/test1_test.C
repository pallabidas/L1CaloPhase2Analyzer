...
void test1_test(){
  ...
  TH1F  *h1 = (TH1F*)fdata1->Get("reso90");
...
//  out1<<"reso90"<<"\t"<<histomean<<"\t"<<histomean_err<<endl;
//  out1.close();
  ...
  double mean = 1.;
  double error = 0.;
  RooRealVar v_m("v_m", "v_m", 0, 0., 100.);
  v_m.setVal(data_hist.mean(x));
  v_m.setRange(data_hist.mean(x) - data_hist.sigma(x), data_hist.mean(x) + data_hist.sigma(x));
  RooRealVar gamma_Z0("gamma_Z0_U", "Z0 width", 2.3, 0., 30., "GeV"); // gamma
  RooRealVar g_w("g_w", "width Gaus", 1., 0. , 30., "GeV"); // sigma
  RooVoigtian voigt("voigt", "Voigtian", x, v_m, gamma_Z0, g_w);
  RooPlot* frame = x.frame(Title("reso90"));
  ...
  c1->SaveAs("/afs/cern.ch/work/p/pdas/www/emulator_phase2/12_3_0_pre4/singlepion/forCalib/reso90_test.pdf");
  c1->SaveAs("/afs/cern.ch/work/p/pdas/www/emulator_phase2/12_3_0_pre4/singlepion/forCalib/reso90_test.png");
