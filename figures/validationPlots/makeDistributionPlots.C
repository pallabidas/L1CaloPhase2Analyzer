...
void makeDistributionPlots(TString sampleName, TString legend, TString inputDirectory)
{
  ...
  // Troubleshooting:
  TString poorMatchCut = "&& ((gct_cPt - genPt)/(genPt) > -11)";  
  ...
  //                         "GCT (p_{T}^{reco} - p_{T}^{gen})/p_{T}^{gen}", bonusDescriptorPoorMatchCut, nBins, -0.5, 0.5, ymaxPtDiff);
  ...
  //                        "RCT (p_{T}^{reco} - p_{T}^{gen})/p_{T}^{gen}", tdrCutDescriptor, nBins, -0.5, 0.5, ymaxPtDiff);
  ...
  singleDistributionPlots("gct_pT_fractional_diff", "(gct_cPt - genPt)/(genPt)", cut + "&& (gct_cPt > 15)", legend, treePath, inputDirectory, outputDirectory,
                         "GCT (p_{T}^{reco} - p_{T}^{gen})/p_{T}^{gen}", tdrCutDescriptor, nBins, -0.5, 0.5, ymaxPtDiff);
